...
import { missions as initialMissions } from "@/mock-data/event";
...
export default function MissionsPage() {
  ...
  const toggle = (id: number) => {
    setMissions((items) => items.map((item) => item.id === id ? { ...item, completed: !item.completed } : item));
  };
  ...
  return (
    <MobileShell title="คะแนนในงาน" backHref="/">
      <section className="missions-screen paper-texture">
        ...
        <div className="progress-track"><i style={{ width: `${completed / missions.length * 100}%` }} /></div>
